  ...
  static const _deviceSetup = DocSection(
    ...
    articles: [
      DocArticle(
        ...
        content: '''Device registration links your phone to your employee account. This ensures only authorized devices can punch attendance. There are two ways to register:

⏺️ Method 1: Manual Registration (Standard)
1. Open the app and go to Settings
2. Enter the Server URL provided by your IT admin
3. Enter your API Key (generated by admin)
4. Tap "Register to Server"
5. The app sends a registration request containing your device's unique hardware ID (Device UUID), device name/model
6. Wait for admin approval

⏺️ Method 2: QR Code Onboarding (Fast)
1. Ask your admin to generate an Onboarding QR code from the admin dashboard
2. In the app Settings, tap "Scan QR to Auto-Setup"
3. Scan the QR code shown by your admin
4. The app automatically configures the server URL, registers the device, and assigns you to a branch — all in one step
5. No manual entry needed!

Approval Flow (Manual Method):
1. Registration request is sent to the server
2. Admin reviews the request in the web dashboard
3. Admin approves the device and assigns it to one or more branches
4. Once approved, the device status changes to "Active"
5. You'll see "Ready to Punch" on the home screen

Device Status Indicators:
• "🔐 Pending Approval" — Waiting for admin to approve your device
• "📋 No Branch Assigned" — Approved but admin hasn't assigned a branch location yet
• "✅ Active & Ready" — Fully configured and ready to punch
• "🚫 Suspended" — Device has been deactivated by admin (contact HR)
• "⚠️ Max Devices Reached" — You've reached the device limit (ask admin to remove an old device)

Important: The server enforces a maximum number of devices per employee (default: 5). If you try to register more, you'll be asked to remove an old device first.''',
      ),
      DocArticle(
        id: 'multi_device',
...
• You need a backup device

Rules:
• Each device registers independently with its own API key
• All devices are linked to the same Employee ID
• Punches from any device are recorded under your name
• The server limits the number of active devices (default maximum: 5)
• Admin can suspend individual devices if needed (e.g., lost phone)
• Only one device needs to be marked as "Active" at a time (admin-controlled)

To add another device:
1. Install the app on the new device
2. Get a new API key from your admin
3. Enter the server URL and API key in Settings
4. Register the device
5. If you've reached the device limit, ask admin to remove an old device

To remove a device:
• Ask your admin to unbind it from the admin dashboard
• The device will immediately be blocked from punching
• Your other devices continue to work normally

Device Labels:
When registering, you can give your device a label (e.g., "John's Samsung A55" or "Work Tablet"). This helps admin identify which device is which in the dashboard.''',
      ),
    ],
  );
...
3. Some branches also have multiple "checkpoints" — specific clock-in points like "Main Gate", "Building A Entrance", or "Parking Lot" with their own GPS coordinates and radius
4. When you punch, the app checks if your GPS location falls within any assigned branch's zone OR any of its checkpoints
5. If you're outside all zones and checkpoints, the punch is rejected (for punch types that require geofencing)

Multiple Checkpoints Per Branch:
Large office sites may have several designated clock-in areas:
• Main entrance gate — for everyone entering through the front
• Building A lobby — for employees in the east wing
• Parking lot kiosk — for those who arrive by car
• Factory floor entrance — for production staff
Each checkpoint has its own allowed radius (typically smaller than the overall branch radius).

Distance Display:
The punch screen shows your distance from the nearest branch or checkpoint. You'll see:
• "📍 You're X meters from [Branch Name]" — within zone, ready to punch
• "📍 You're X meters from [Branch Name] (Main Gate)" — within a checkpoint zone
...
• If you get "outside geofence" errors, try moving to a different checkpoint area
...
The app checks your device's security status and shows it on the Dashboard:
• 🟢 "Device Secure" — No root access detected. You can punch normally.
• 🔴 "Device Compromised" — Root access (su binary) detected. Punching is blocked. Contact your IT admin.

Important: An unlocked bootloader is NOT treated as a compromise. Custom ROMs and developer devices with unlocked bootloaders can still use the app normally. Only actual root access (ability to run su commands) triggers the security block.

Tap the security status card on your Dashboard to learn more.''',
      ),
      DocArticle(
